...
end_packet_on_channel_event(Simulation_Run_Ptr simulation_run, void * c_ptr)
{
  ...
  /* See if there is a packet waiting in the queue. */
  if(fifoqueue_size(sim_data->bucket_queue) > 0) {
    /* Yes, there is a packet waiting. Remove it from the queue and
       place it in the now free channel. */
    this_packet = (Packet_Ptr) fifoqueue_get(sim_data->bucket_queue);
    this_packet->channel = channel;
    server_put(channel, (void*) this_packet);

    /* Schedule the end of this packet on the channel. */
